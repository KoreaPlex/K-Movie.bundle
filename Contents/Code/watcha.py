...
try:
    import requests
    plex = False
    try:Log.Info('requests imported')
    except:pass
except:
    import urllib, unicodedata, traceback, re
    plex = True
    try:Log.Info('requests not imported')
    except:pass
from time import sleep

def watcha_find(results, media, lang, manual=False ):
    name = media.name
    year = media.year
    search_result = watcha(keyword=name , year = year)
    try:
        data = search_result.info['API_INFO']
        results.Append(MetadataSearchResult(id='watcha_%s' % data['code'], name='WATCHA  :  %s' % data['title'], year=data['year'], score=10, lang=lang))
    except:
        pass
...
if __name__ == '__main__':
    a = watcha(keyword='Kwon Ji Yong (권지용) Act III: Motte' ,year=None , media_type='movies')
    a